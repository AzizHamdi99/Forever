...
useEffect(() => {
  if (singleProduct && singleProduct.images && singleProduct.images.length > 0) {
    setMainPic(singleProduct.images[0]); 
  }
}, [singleProduct]);

const handleSizeClick = (item) => {
  setSize(item)
  setPrd({ ...prd, productid: singleProduct._id, size: item }) // Immediately update prd when size is selected
}
        ...
        <div className='ml-2'>
          ...
          <div className='flex items-center gap-10 my-6'>
            {singleProduct.sizes.map((item)=>{
              return(
                <div onClick={()=>handleSizeClick(item)} className={size===item?"w-[61px] cursor-pointer h-[61px] border-[2px] border-[#FF8551]  bg-[#FBFBFB] text-center ":"w-[61px] cursor-pointer h-[61px] border-[1px] border-[#EBEBEB]  bg-[#FBFBFB] text-center "}>
          ...
          <button onClick={handleSubmit} className='bg-black text-white text-[16px] font-[600] px-11 py-3 my-4'>ADD TO CART</button>
