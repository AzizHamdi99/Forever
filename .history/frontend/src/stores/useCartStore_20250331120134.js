...
export const useCartStore = create((set, get) => ({
    ...
    removeFromCart: async ({ productid, size }) => {
        try {
            // Corrected DELETE request with data sent properly
            await axiosInstance.delete(`/cart`, {
                data: { productid, size }  // DELETE requests need data in the `data` field
            });

            // Update state correctly by removing the exact item
            set((prevState) => ({
