...
function AdminOrders() {
  const {allOrders,getAllorders,updateProcess}=useOrderStore()
  ...
  <div className="h-[2px] bg-black w-10">
      ...
      <div className='flex  flex-col gap-5 text-[#494949] mt-5'>
        {allOrders.map((order,index)=>{
          ...
          <div className=''>
            <select onChange={(e)=>handelChange({orderId:order._id,process:e.target.value})} value={ value={order.process}} className='font-medium border-[1px] p-2 shadow-md' name="" id="">
              {process.map((p,i)=>{
                return(
