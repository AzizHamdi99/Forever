...
function Navbar() {
    ...
    /*useEffect(() => {
      if (isChekingAuth) return; // Don't run the check while the authentication check is in progress
      chekingAuth(); // Call the check if the user is not already loaded
    }, [isChekingAuth, chekingAuth]);*/
       ...
       <div className='text-white cursor-pointer bg-black py-2 px-4 rounded-full' onClick={()=>navigate("/login")}>
        ...
        <div className='flex gap-4'>
          {user.role==="admin" &&<ShieldUser className=' cursor-pointer' onClick={()=>navigate('/admin')}  />}
            ...
            <img onClick={()=>navigate("/cart")} className='w-[24px] h-[24px] cursor-pointer' src={assets.cart_icon} alt="" />
