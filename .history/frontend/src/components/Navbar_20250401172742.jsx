...
function Navbar() {
    ...
    return (
        <div>
            ...
            {showSearch ?
                <div className='mx-32 bg-[#f9fbfc] flex items-center justify-center py-8 gap-3'>
                    <div className='flex border-[1px] items-center justify-between rounded-3xl bg-[#f9fbfc] border-gray-400 px-4 py-2 w-[600px] '>
                        <input value={searchTerm} onChange={()=>setSearchTerm(e.target.value)} className='outline-none w-full text-gray-400 bg-[#f9fbfc] ' placeholder='Search' type="text" name="" id="" />
            ...
            {showLogout &&
                <div ref={logoutRef} className='text-[18px] text-[#5B5B5B] absolute right-40 bg-[#F8F8F8] px-3 w-[150px] h-[100px] text-left rounded-[4px]'>
                    <p className='my-3 cursor-pointer' onClick={() => navigate("/orders")}>Orders</p>
                    <p onClick={loginout} className=' cursor-pointer'>logout</p>
                </div>
            }
        </div>
    )
