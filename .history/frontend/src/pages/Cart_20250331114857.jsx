...
function Cart() {
  ...
  useEffect(()=>{
getCartItems()
  },[getCartItems,removeFromCart,addToCart,decreaseQuantity])
  console.log(cart)
    ...
    <div>
      {cart?.map((product,index)=>(
        <div key={index} className='flex flex-col gap-3'>
          ...
          <img onClick={()=>removeFromCart({productid:product._id,size:product.size})} className='w-[20px] h-[20px]' src={assets.bin_icon} alt="" />
          </div>
