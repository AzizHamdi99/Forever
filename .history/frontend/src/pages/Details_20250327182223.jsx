...
function Details() {
   const{getSingleProduct,singleProoduct,products}=useAdminStore()
...
useEffect(() => {
  console.log(singleProoduct);
}, [singleProoduct]);
//const [mainPic,setMainPic]=useState(singleProoduct.images[0])
if (!singleProoduct) {
  return <div>Loading...</div>; // Prevents rendering undefined properties
}
  ...
  return (
    ...
    <div className='mt-10 mx-32'>
      <div className='flex gap-4 '>
        ...
        <div className='ml-2'>
          <p className='text-[34px] font-[500] text-[#3D3D3D] mb-2'>{singleProoduct.name}</p>
          ...
          <p className='text-[32px] text-[#2A2A2A] font-[500] '>${singleProoduct.price}</p>
          <p className='text-[#555555] text-[16px] font-[400] my-6'>{singleProoduct.description}</p>
          <p className='text-[#656565] text-[20px] font-[600]'>Select Size</p>
          <div className='flex items-center gap-10 my-6'>
            {singleProoduct.sizes.map((item)=>{
              return(
                <div onClick={()=>setSize(item)} className={size===item?"w-[61px] cursor-pointer h-[61px] border-[2px] border-[#FF8551]  bg-[#FBFBFB] text-center ":"w-[61px] cursor-pointer h-[61px] border-[1px] border-[#EBEBEB]  bg-[#FBFBFB] text-center "}>
                  <p className='mt-4 text-[#1D1D1D] text-[16px] font-[400]'>
                    {item}
      ...
      <div className='mt-20 mb-36'>
        ...
        <div className='grid grid-cols-5 gap-5 mt-3 font-outfit '>
          {products.filter((item)=>item.category===singleProoduct.category&&item._id!==singleProoduct._id).slice(0,5).map((item)=>{
            return(
              <div onClick={()=>navigate(`/product/${item._id}`)}  className='text-[#494949] cursor-pointer'>
                <img className='w-[280px] h-[280px] mb-4' src={item.image[0]} alt="" />
