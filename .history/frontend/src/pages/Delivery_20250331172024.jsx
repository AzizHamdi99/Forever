...
function Delivery() {
    const{total,subTotal,TVA,calculateTotals}=useCartStore()
    useEffect(()=>{
calculateTotals()
    },[calculateTotals])
...
<div className='flex justify-between mt-5'>
  <div className=' flex flex-col gap-4 '>
    ...
    <input type="text" onChange={(e)=>setInfo({...info.address,street:e.target.value})} placeholder='Street'  className='h-[50px] border-[1px] border-[#C5C5C5] px-3 rounded-[4px] text-[#8B8B8B] text-[18px]'/>
    <div className=' flex items-center gap-4 '>
    <input type="text"  onChange={(e)=>setInfo({...info.address,city:e.target.value})}  placeholder='City'  className='h-[50px] border-[1px] border-[#C5C5C5] px-3 rounded-[4px] text-[#8B8B8B] text-[18px]'/>
    <input type="text" placeholder='State'  className='h-[50px] border-[1px] border-[#C5C5C5] px-3 rounded-[4px] text-[#8B8B8B] text-[18px]'/>
    </div>
    <div className=' flex items-center gap-4 '>
    <input type="number"  onChange={(e)=>setInfo({...info.address,zipCode:e.target.value})} placeholder='Zip code'  className='h-[50px] border-[1px] border-[#C5C5C5] px-3 rounded-[4px] text-[#8B8B8B] text-[18px]'/>
    <input type="text"  onChange={(e)=>setInfo({...info.address,country:e.target.value})} placeholder='Country' className='h-[50px] border-[1px] border-[#C5C5C5] px-3 rounded-[4px] text-[#8B8B8B] text-[18px]' />
    </div>
  ...
  <button onClick={()=>navigate('/orders')} className='text-[14px] ml-auto mt-2 font-[500]  bg-black text-white w-1/2 py-4 uppercase items-center '> Place Order</button>
  </div>
